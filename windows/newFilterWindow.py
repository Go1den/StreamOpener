...
class NewFilterWindow:
    ...
    def initializeWindow(self):
        ...
        self.window.title(LabelConstants.LABEL_NEW_FILTER_WINDOW)
    ...
    def addEntryFrame(self):
        labelInstructions = Label(self.entryFrame, text=LabelConstants.LABEL_NEW_FILTER_INSTRUCTIONS)
        labelInstructions.grid(row=0, column=0, columnspan=2, sticky=W, padx=4, pady=4)
        labelStream = Label(self.entryFrame, text=LabelConstants.LABEL_STREAMER)
        labelStream.grid(row=1, column=0, sticky=E, padx=4, pady=4)
        self.comboboxStream = Combobox(self.entryFrame, values=self.parent.parent.teams[LabelConstants.LABEL_ALL_TEAM], state="readonly")
        self.comboboxStream.grid(row=1, column=1, sticky=W, padx=(0, 4), pady=4)
        labelGame = Label(self.entryFrame, text=LabelConstants.LABEL_GAME)
    ...
    def ok(self):
        ...
        if not game and not streamer:
            messagebox.showerror(LabelConstants.LABEL_ERROR, MessageConstants.MSG_NO_FILTER_INFO_PROVIDED)
        if game and not isRecognizedTwitchGame(self.parent.parent.credentials.oauth, game):
            messagebox.showerror(LabelConstants.LABEL_ERROR, MessageConstants.MSG_INVALID_GAME)
        else:
            newFilter = {}
            if streamer:
            ...
            if streamer and game:
                ...
                key = LabelConstants.LABEL_FILTER_KEY_COMBINED
            elif streamer:
                newFilter["description"] = streamer
                key = LabelConstants.LABEL_FILTER_KEY_STREAMER
            else:
                newFilter["description"] = game
                key = LabelConstants.LABEL_FILTER_KEY_GAME
